...
int main(){
  ...
  int marker = 0;
  for (i = 1; i <= 10; i++){
    SmithyFunction(player1, game1, handPos);
    // printf("cards in hand %d\n", game1->handCount[player1]);
    int j;
    // discards 3 cards from hand and adds 3 cards to deck to make sure there's
    // enough cards to draw in actionSmithy function
    for (j = 0; j < 3; j++){
      discardCard(handPos, player1, game1, 0);
      gainCard(i, game1, 1, player1);
    }
    printf("player 0 should have %d in hand and has %d\n", 5 - i, game1->handCount[player1]);
    if (game1->handCount[player1] != 5 - i){
      marker++;
    }
  }
  if (marker == 0){
    printf("All test passed unittest2\n");
  }
  else {
    printf("unittest2 failed %d out of 10 tests\n", marker);
  }
  return 0;
}
